...
router.post('/edit',wrap(function *(req,res){
    let body = req.body;
    let person = req.session.user;
    let p = yield User.findOne({uID:person.uID}).exec();
    if(!p){
        throw Error(4002);
    }
    ...
    let up = yield update(User,{uID:p.uID},{$set:body});
    if(up.no != 200){
        res.send({no:500,msg:"更新用户信息失败!"})
    }else{
        res.send(Object.assign({no:200},body));
    }
}))
